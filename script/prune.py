...
from channel_vis import image_test
...
class Prune(object):
    def __init__(self, net):
        self._net = net
        self.conv_data = {}
        self.ratio = 56                 # 裁剪掉多少卷积核
        self.base_layer = 'conv4_1_1b'    # 以哪一层为基础进行裁剪 一般是带b的层

    def init_layer(self, name):         # init a layer
    ...
    def _prune(self, name, conv_param, del_kernels=None, not_del_filters=False, del_filters=None):
        ...
        if del_filters is not None:
            ...
            if not not_del_filters:
                ...
                del_filters = np.where(abs_mean < 1e-3)[0]
                weight = np.delete(weight, del_filters, axis=0)
                bias = np.delete(bias, del_filters, axis=0)
            else:
    ...
    def _prune_block5(self, name, conv_param, del_kernels=None, del_filters=None):
        weight, bias = conv_param
        weight = weight.data
        bias = bias.data
        origin_channels = weight.shape[0]
        ...
        print("{} layer output channels is {}".format(name,weight.shape))
    ...
    def prune_conv(self, name, bottom=None):        # prune block1 and block2
        if bottom is None:
    ...
    def prune_conv_block5(self, name, bottom=None, not_del_filters=False):      # prune block5
        if bottom is None:
            self.conv_data[name] = self._prune(name, self._net.params[name])
        else:
            if not_del_filters is True:  # filters不需要裁剪(output), 但是kernels需要裁剪(input)
                self.conv_data[name] = self._prune_block5(name, self._net.params[name],
...
root = "../my_model/v2/"
prototxt = root + "TestModel.prototxt"
caffemodel = root + "TestModel.caffemodel"
...
pruner.prune_conv_block5('conv5_1_1b')
pruner.prune_conv_block5("conv5_1_2", "conv5_1_1", )
pruner.prune_conv_block5("conv5_2_1", "conv5_1_2", not_del_filters=True)
pruner.prune_conv_block5("conv5_2_2", "conv5_2_1", )
pruner.prune_conv_block5("conv5_3_1", "conv5_2_2",  not_del_filters=True)
pruner.prune_conv_block5("conv5_3_2", "conv5_3_1", )
pruner.prune_conv_block5("conv5_4_1", "conv5_3_2",  not_del_filters=True)
pruner.prune_conv_block5("conv5_4_2", "conv5_4_1", )
pruner.prune_conv_block5("conv5_5_1", "conv5_4_2",  not_del_filters=True)
pruner.prune_conv_block5("conv5_5_2", "conv5_5_1", )
pruner.prune_conv_block5("conv5_6_1", "conv5_5_2",  not_del_filters=True)
pruner.prune_conv_block5("conv5_6_2", "conv5_6_1", )
pruner.prune_concat('fc_svd_v', ('conv5_2_2', 'conv5_4_2', 'conv5_6_2'))


pro_new = root + "TestModel_prune.prototxt"
pruner.save(pro_new, root + 'TestModel_prune.caffemodel')
